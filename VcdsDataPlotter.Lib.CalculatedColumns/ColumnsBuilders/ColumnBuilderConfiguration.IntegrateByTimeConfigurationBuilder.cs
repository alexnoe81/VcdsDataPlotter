...
namespace VcdsDataPlotter.Lib.CalculatedColumns.ColumnsBuilders
{
    ...
    {
		private class IntegrateByTimeConfigurationBuilder : CalculatedColumnBuilderConfiguration
		{
			public IntegrateByTimeConfigurationBuilder(ColumnIdentity identity, ColumnBuilderConfiguration sourceColumnBuilderConfiguration)
				: base(identity, [sourceColumnBuilderConfiguration])
			{
			}

			public override string ToString() => $"[{Identity}] := Integrate over [{SourceColumnBuilderConfigurations.First()}]";
			...
			public override ColumnBuilderBuildingResult TryBuild(IList<IDiscreteDataColumn> sourceColumns, [NotNullWhen(true)] out IDiscreteDataColumn? result)
			{
				...
				result = IntegralByTimeColumn.Create(Identity.Title, Identity.ChannelId, resolvedSourceColumns.First());
